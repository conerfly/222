...
// 	}
	...
	try
    {
        ...
        hr = pConnection->Open((_bstr_t)m_strConn,"","",NULL);   //Connect a DataBase
...
bool ADOConn::GetSQLFile(char *tablename,char *itemname,CFile *pFile)
{
    ...
    {	
        //Open a connection	
        pConnection.CreateInstance(__uuidof(Connection));		
        hr = pConnection->Open((_bstr_t)m_strConn,"","",NULL);        		
        pRs.CreateInstance(__uuidof(Recordset));		
        pRs->Open(tablename,_variant_t((IDispatch *) pConnection,true),adOpenKeyset,adLockOptimistic,adCmdTable);		
		//read  data  	
		long lDataLength = pRs->Fields->GetItem(itemname)->ActualSize;	
		varBLOB = pRs->GetFields()->GetItem(itemname)->GetChunk(lDataLength);		
